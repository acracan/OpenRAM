...
def inst_accesstx(stim_file, dbits):
    ...
    for i in range(dbits):
        pmos_access_string="mp{0} DATA[{0}] WEb_trans D[{0}]_buf {1} {2} w={3}u l={4}u\n"
...
def gen_pwl(stim_file, sig_name, clk_times, data_values, period, slew):
...
def gen_data(stim_file, clk_times, sig_name, period, slew):
    ...
    gen_pwl(stim_file, sig_name, clk_times, values, period, slew)
...
def gen_addr(stim_file, clk_times, addr, period, slew):
    ...
    for i in range(len(addr)):
        ...
        if addr[i]==1:
            gen_pwl(stim_file, sig_name, clk_times, ones_values, period, slew)
        else:
            gen_pwl(stim_file, sig_name, clk_times, zero_values, period, slew)            
...
def gen_csb(stim_file, clk_times, period, slew):
    ...
    gen_pwl(stim_file, "CSb", clk_times, values, period, slew)

def gen_web(stim_file, clk_times, period, slew):
    """ Generates the PWL WEb signal"""
    # values for NOP, W1, W0, W1, R0, W1, W0, R1, NOP
    values = [1, 0, 0, 0, 1, 0, 0, 1, 1]
    gen_pwl(stim_file, "WEb", clk_times, values, period, slew)
    
def gen_oeb(stim_file, clk_times, period, slew):
    """ Generates the PWL WEb signal"""
    # values for NOP, W1, W0, W1, R0, W1, W0, R1, NOP
    values = [1, 1, 1, 1, 0, 1, 1, 0, 1]
    gen_pwl(stim_file, "OEb", clk_times, values, period, slew)

def gen_web_trans(stim_file, clk_times, period, slew):
    """ Generates the PWL transmission gatesignal"""
    # values for NOP, W1, W0, W1, R0, W1, W0, R1, NOP
    values = [1, 0, 0, 0, 1, 0, 0, 1, 1]
    gen_pwl(stim_file, "WEb_trans", clk_times, values, period, slew)
    values = [0, 1, 1, 1, 0, 1, 1, 0, 0]
    gen_pwl(stim_file, "WEb_trans_inv", clk_times, values, period, slew)
